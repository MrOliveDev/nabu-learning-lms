...
var itemClick = function (event) {
    ...
    switch ($(this).attr("data-type")) {
        ...
        case "4":
            $.get({
                url: baseURL + "/user/" + $(this).attr('id').split('_')[1],
        ...
        case "training":
            $.get({
                url: baseURL + "/training/" + $(this).attr('id').split('_')[1],
                success: function (data) {
...
var editUser = function (e) {
    ...
    $.post({
        url: baseURL + "/setSessionForUserPage",
        data: {
            "data": parentId
        }
    }).then(function (data) {
...
async function download_person_pdf(filename) {
...
async function download_group_pdf(filename) {
    const a = document.createElement('a');
    a.download = filename;
...
$(document).ready(function () {
    ...
    $('.counter').each(function () {
        ...
        jQuery({
            ...
            Counter: $this.next(".store-value").val()
        }, {
            duration: 2000,
            easing: 'swing',
            step: function () {
    ...
    $('.table-group-document>table').map(function () {
        $(this).DataTable({
            "processing": true,
            "serverSide": true,
            "responsive": true,
            ...
            "pageLength": 50,
            "ajax": {
                "url": baseURL + "/getGroupDocumentBySession",
                "dataType": "json",
                "type": "POST",
                "data": {
                    _token: $('meta[name="csrf-token"]').attr('content'),
                    session_id: $(this).parents(".table-group-document").attr('id').split('_')[1]
                }
            },
            "columns": [{
                    "data": "groupname"
    ...
    $('.table-person-document>table').map(function () {
        $(this).DataTable({
            "processing": true,
            "serverSide": true,
            "responsive": true,
            ...
            "pageLength": 50,
            "ajax": {
                "url": baseURL + "/getPersonDocumentBySession",
                "dataType": "json",
                "type": "POST",
                "data": {
                    _token: $('meta[name="csrf-token"]').attr('content'),
                    session_id: $(this).parents(".table-person-document").attr('id').split('_')[1]
                }
            },
            "columns": [{
                    "data": "studentname"
