...
Abstract Class dbModel {
    ...
    public function __construct() {
        try {
            $this->db = new PDO(env('DB_DATABASE'), env('DB_USERNAME'), env('DB_PASSWORD'), array(PDO::MYSQL_ATTR_INIT_COMMAND => 'SET NAMES utf8'));
    ...
    public function createDir($root, $folder) {
        $path = env('SERVER_ROOT') . $root . $folder;
    //echo "path:".$path."\n";
        if (!file_exists($path)){
            $make = mkdir($path, 0777);
        }
        return !$make ? array("error"=>"Unable to create the folder '$folder' in '".env('ROOTPATH') . $root."'"):true;
    }
