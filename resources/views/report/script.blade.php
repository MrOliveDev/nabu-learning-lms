                    ...
                    if(info.trainings[i].lessons && info.trainings[i].lessons.length > 0){
                        info.trainings[i].lessons.forEach(lessonData => {
                            if(lessonData && lessonData.lesson && lessonData.lesson.name)
                                $(`#overviewPane #training_lessons_${i}`).append(`<tr><td style="color:#000000; border:1px solid #000000;">${lessonData.lesson.name}</td></tr>`);
                            else{
                                console.log('Empty lesson name:', lessonData);
                                $(`#overviewPane #training_lessons_${i}`).append(`<tr><td style="color:#000000; border:1px solid #000000;"></td></tr>`);
                            }
                        })
                    }
                }
                    ...
                    trainingData.lessons.forEach(lessonData => {
                        if(lessonData && lessonData.lesson){
                            ...
                            if(lessonData.optim && lessonData.lesson.threshold_score <= lessonData.optim.progress_screen_optim)
                                ...
                                $(`#overviewPane #training_synthetic_${i} #lesson_status`).html(`Not Completed`);
                            if(lessonData.optim)
                                $(`#overviewPane #training_synthetic_${i} #lesson_progress`).html(`${lessonData.optim.progress_screen_optim}%`);
                            else
                                $(`#overviewPane #training_synthetic_${i} #lesson_progress`).html(`0%`);
                            $(`#overviewPane #training_synthetic_${i} #screens_count`).html(`${lessonData.screensCount}`);
                            $(`#overviewPane #training_synthetic_${i} #chapters_count`).html(`${lessonData.chaptersCount}`);
                            $(`#overviewPane #training_synthetic_${i} #visited_count`).html(`${lessonData.visitedScreens}`);
                            $(`#overviewPane #training_synthetic_${i} #time_spent`).html(`${lessonData.time_module}`);
                            if(lessonData.last_eval){
                                $(`#overviewPane #training_synthetic_${i} #eval_score`).html(`${lessonData.last_eval.note}% (${lessonData.last_eval.status})`);
                                if(lessonData.last_eval.status == 'failed')
                                    $(`#overviewPane #training_synthetic_${i} #eval_score`).css("background", "red");
                                else
                                    $(`#overviewPane #training_synthetic_${i} #eval_score`).css("background", "green");
                            }
                            else if(lessonData.first_eval){
                                $(`#overviewPane #training_synthetic_${i} #eval_score`).html(`${lessonData.first_eval.note}% (${lessonData.first_eval.status})`);
                    ...
                    trainingData.lessons.forEach(lessonData => {
                        if(lessonData && lessonData.lesson){
                            ...
                            if(lessonData.optim && lessonData.lesson.threshold_score <= lessonData.optim.progress_screen_optim)
                                ...
                                $(`#overviewPane #training_complete_${i} #lesson_status`).html(`Not Completed`);
                            if(lessonData.optim)
                                $(`#overviewPane #training_complete_${i} #lesson_progress`).html(`${lessonData.optim.progress_screen_optim}%`);
                            else
                                $(`#overviewPane #training_complete_${i} #lesson_progress`).html(`0%`);
                            $(`#overviewPane #training_complete_${i} #screens_count`).html(`${lessonData.screensCount}`);
                            $(`#overviewPane #training_complete_${i} #chapters_count`).html(`${lessonData.chaptersCount}`);
                            $(`#overviewPane #training_complete_${i} #visited_count`).html(`${lessonData.visitedScreens}`);
                            $(`#overviewPane #training_complete_${i} #time_spent`).html(`${lessonData.time_module}`);
                            if(lessonData.last_eval){
                                $(`#overviewPane #training_complete_${i} #eval_score`).html(`${lessonData.last_eval.note}% (${lessonData.last_eval.status})`);
                                if(lessonData.last_eval.status == 'failed')
                                    $(`#overviewPane #training_complete_${i} #eval_score`).css("background", "red");
                                else
                                    $(`#overviewPane #training_complete_${i} #eval_score`).css("background", "green");
                            }
                            else if(lessonData.first_eval){
                                $(`#overviewPane #training_complete_${i} #eval_score`).html(`${lessonData.first_eval.note}% (${lessonData.first_eval.status})`);
                                if(lessonData.first_eval.status == 'failed')
                                    $(`#overviewPane #training_complete_${i} #eval_score`).css("background", "red");
                                else
                                    $(`#overviewPane #training_complete_${i} #eval_score`).css("background", "green");
                                ...
                                $(`#overviewPane #training_complete_${i} #complete_eval_score`).remove();

                            if(lessonData.lessonCourse && lessonData.lessonCourse.module_structure){
                                let structure = JSON.parse(lessonData.lessonCourse.module_structure);
                                ...
                                if(structure && structure.length > 0){
                                    ...
                                    structure.forEach(screen => {
                                        if(screen){
                                            if(screen.type == "item" || screen.nav == "true"){
                                                let screentitle = '', screenstatus = '', viewcount = 0, viewtime = '00:00:00';
                                                if(progress_details[screen.id]){
                                                    screenstatus = progress_details[screen.id]['status'];
                                                    viewcount = progress_details[screen.id]['visu'];
                                                    viewtime = progress_details[screen.id]['time'];
                                                }
                                                let found = screen_titles.find(screenitem => screenitem && screenitem.id == screen.id);
                                                if(found)
                                                    screentitle = found.title;
                                                
                                                $(`#overviewPane #training_complete_screens_${i}`).append("<tr>" + 
                    ...
                    trainingData.lessons.forEach(lessonData => {
                        if(lessonData && lessonData.lesson){
                            ...
                            if(lessonData.optim && lessonData.lesson.threshold_score <= lessonData.optim.progress_screen_optim)
                                ...
                                $(`#overviewPane #training_evaluation_${i} #lesson_status`).html(`Not Completed`);
                            if(lessonData.optim)
                                $(`#overviewPane #training_evaluation_${i} #lesson_progress`).html(`${lessonData.optim.progress_screen_optim}%`);
                            else
                                $(`#overviewPane #training_evaluation_${i} #lesson_progress`).html(`0%`);
                            $(`#overviewPane #training_evaluation_${i} #screens_count`).html(`${lessonData.screensCount}`);
                            $(`#overviewPane #training_evaluation_${i} #chapters_count`).html(`${lessonData.chaptersCount}`);
                            $(`#overviewPane #training_evaluation_${i} #visited_count`).html(`${lessonData.visitedScreens}`);
                            $(`#overviewPane #training_evaluation_${i} #time_spent`).html(`${lessonData.time_module}`);
                            if(lessonData.last_eval){
                                $(`#overviewPane #training_evaluation_${i} #eval_score`).html(`${lessonData.last_eval.note}% (${lessonData.last_eval.status})`);
                                if(lessonData.last_eval.status == 'failed')
                                    $(`#overviewPane #training_evaluation_${i} #eval_score`).css("background", "red");
                                else
                                    $(`#overviewPane #training_evaluation_${i} #eval_score`).css("background", "green");
                            }
                            else if(lessonData.first_eval){
                                $(`#overviewPane #training_evaluation_${i} #eval_score`).html(`${lessonData.first_eval.note}% (${lessonData.first_eval.status})`);
                                if(lessonData.first_eval.status == 'failed')
                                    $(`#overviewPane #training_evaluation_${i} #eval_score`).css("background", "red");
                                else
                                    $(`#overviewPane #training_evaluation_${i} #eval_score`).css("background", "green");
                            ...
                            if(lessonData.eval_questions && lessonData.eval_questions.length > 0){
                                ...
                                lessonData.eval_questions.forEach((questionData, j) => {
                                    ...
                                    $(`#overviewPane #evaluation_details_${j} #question_title`).html(`<b>${questionData.title}</b>`);
                                    if(questionData.options && questionData.options.length > 0){
                                        questionData.options.forEach((option, k) => {
                                            $(`#overviewPane #evaluation_details_${j}`).append("<tr>" + 
                                                ...
                                                `<td style="width: 10%;` + (option.attendu == "1" ? ` background: lightgreen;` : ``) + ` border:1px solid #000000;" ></td>` + 
                                                `<td style="width: 10%;` + (option.recu == "1" ? (option.ok ? ` background: lightgreen;` : ` background: red;`) : ``) + ` border:1px solid #000000;" ></td>` + 
                                            "</tr>");
                                        });
                                        $(`#overviewPane #evaluation_details_${j}`).append("<tr>" + 
                                            `<td style="width: 30%; color:#000000; border:1px solid #000000; background: ` + (questionData.result ? 'lightgreen' : 'red') + `;">Result</td>` + 
                    ...
                    trainingData.lessons.forEach(lessonData => {
                        if(lessonData && lessonData.lesson){
                            ...
                            if(lessonData.optim && lessonData.lesson.threshold_score <= lessonData.optim.progress_screen_optim)
                                ...
                                $(`#overviewPane #complete_evaluation_${i} #lesson_status`).html(`Not Completed`);
                            if(lessonData.optim)
                                $(`#overviewPane #complete_evaluation_${i} #lesson_progress`).html(`${lessonData.optim.progress_screen_optim}%`);
                            else
                                $(`#overviewPane #complete_evaluation_${i} #lesson_progress`).html(`0%`);
                            $(`#overviewPane #complete_evaluation_${i} #screens_count`).html(`${lessonData.screensCount}`);
                            $(`#overviewPane #complete_evaluation_${i} #chapters_count`).html(`${lessonData.chaptersCount}`);
                            $(`#overviewPane #complete_evaluation_${i} #visited_count`).html(`${lessonData.visitedScreens}`);
                            $(`#overviewPane #complete_evaluation_${i} #time_spent`).html(`${lessonData.time_module}`);
                            if(lessonData.last_eval){
                                $(`#overviewPane #complete_evaluation_${i} #eval_score`).html(`${lessonData.last_eval.note}% (${lessonData.last_eval.status})`);
                                if(lessonData.last_eval.status == 'failed')
                                    $(`#overviewPane #complete_evaluation_${i} #eval_score`).css("background", "red");
                                else
                                    $(`#overviewPane #complete_evaluation_${i} #eval_score`).css("background", "green");
                            }
                            else if(lessonData.first_eval){
                                $(`#overviewPane #complete_evaluation_${i} #eval_score`).html(`${lessonData.first_eval.note}% (${lessonData.first_eval.status})`);
                                if(lessonData.first_eval.status == 'failed')
                                    $(`#overviewPane #complete_evaluation_${i} #eval_score`).css("background", "red");
                                else
                                    $(`#overviewPane #complete_evaluation_${i} #eval_score`).css("background", "green");
                                ...
                                $(`#overviewPane #complete_evaluation_${i} #complete_eval_score`).remove();
                                    ...
                                    structure.forEach(screen => {
                                        if(screen){
                                            if(screen.type == "item" || screen.nav == "true"){
                                                ...
                                                $(`#overviewPane #complete_evalscreens_${i}`).append("<tr>" + 
                                                        `<td style="width:50%;color:#000000; border:1px solid #000000;">${screentitle}</td>` + 
                                                        `<td style="width:15%;color:#000000; border:1px solid #000000;">${screenstatus}</td>` + 
                                                        `<td style="width:20%;color:#000000; border:1px solid #000000;">${viewtime}</td>` + 
                                                        `<td style="width:15%;color:#000000; border:1px solid #000000;">${viewcount}</td>` + 
                            ...
                            if(lessonData.eval_questions && lessonData.eval_questions.length > 0){
                                ...
                                lessonData.eval_questions.forEach((questionData, j) => {
                                    ...
                                    $(`#overviewPane #complete_evaldetails_${j} #question_title`).html(`<b>${questionData.title}</b>`);
                                    if(questionData.options && questionData.options.length > 0){
                                        questionData.options.forEach((option, k) => {
                                            $(`#overviewPane #complete_evaldetails_${j}`).append("<tr>" + 
                                                `<td style="width: 30%; color:#000000; border:1px solid #000000;" >Option ${k + 1}</td>` + 
                                                `<td style="width: 50%; color:#000000; border:1px solid #000000;" >${option.intitule}</td>` + 
                                                `<td style="width: 10%;` + (option.attendu == "1" ? ` background: lightgreen;` : ``) + ` border:1px solid #000000;" ></td>` + 
                                                `<td style="width: 10%;` + (option.recu == "1" ? (option.ok ? ` background: lightgreen;` : ` background: red;`) : ``) + ` border:1px solid #000000;" ></td>` + 
