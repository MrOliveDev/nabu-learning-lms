<script>
    $(document).ready(function() {
        ...
        $("#mailcheck_all").on('click', function() {
            if ($(this)[0].checked)
    ...
    async function previewMail(userId) {
        ...
        if (info == null) {
            swal.fire({
                title: "Warning",
                text: "Error while getting user info.",
                icon: "error",
                confirmButtonText: `OK`
            });
            return;
    ...
    function sendNow() {
        if ($("#from-address").val() == "") {
            swal.fire({
                title: "Warning",
                text: "From Address cannot be empty.",
                icon: "error",
                confirmButtonText: `OK`
            });
            return;
        ...
        if ($("#to-address").val() == "") {
            swal.fire({
                title: "Warning",
                text: "To Address cannot be empty.",
                icon: "error",
                confirmButtonText: `OK`
            });
            return;
        ...
        if ($("#send-subject").val() == "") {
            swal.fire({
                title: "Warning",
                text: "Mail Subject cannot be empty.",
                icon: "error",
                confirmButtonText: `OK`
            });
            return;
        ...
        $.ajax({
            ...
            data: {
                ...
                content: $("#overviewPane").trumbowyg('html'),
                userId: curUser
            },
            success: function(res) {
                if (res.success) {
                    ...
                    notification(res.message, 2);
                    insertHistory($("#process").val(), $("#to-address").val() + ' -------- Error(' + res
                        .message + ')\n');
                }
                swal.close();
            },
            error: function(err) {
                notification("Sorry, You have an error!", 2);
                swal.close();
                insertHistory($("#process").val(), $("#to-address").val() +
                ' -------- Error(API Failed)\n');
            }
        });
    ...
    function delMails() {
        if ($(".sendcheck:checked").length == 0) {
            ...
            swal.fire({
                    title: "Warning",
                    html: "Are you sure to delete these histories?",
                    icon: "warning",
                    showCancelButton: true,
                    confirmButtonText: `Yes`,
                    cancelButtonText: `No`,
                ...
                .then((result) => {
                    var ids = [];
                    if (result.value) {
                        $(".mailcheck:checked").map(function(index, item) {
                            ids.push($(item).attr("id").split("_")[1])
                        })

                        $.ajax({
                            url: 'delMultiHistories',
                            method: 'post',
                            data: {
                                ids: ids
                            },
